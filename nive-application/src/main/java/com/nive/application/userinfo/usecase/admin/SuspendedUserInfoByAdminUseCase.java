...
import com.nive.application.userinfo.dto.AdminUserSuspendedDto;
...
/**
 * @author hosikchoi
 * @class SuspendedUserInfoByAdminUseCase
 * @desc 회원 정지 usecase
 * @since 2026-01-06
 */
@Service
...
@Slf4j
public class SuspendedUserInfoByAdminUseCase {
     ...
     */
    @Transactional
    public void suspended(Long id, UserLoginInfo userLoginInfo){

        String context = "[관리자] [회원 정지(삭제)]";
     ...
     */
    @Transactional
    public void suspendedChecked(AdminUserSuspendedDto dto, UserLoginInfo userLoginInfo){

        String context = "[관리자] [회원 선택 정지(삭제)]";
        AdminInfoHelperDto adminUser = userInfoUtilHelper.getVerifiedCurrentAdminInfo(userLoginInfo, context);

        dto.getIds().forEach(id -> {
            User user = userRepository.findById(id).orElse(null);
            if (user == null) {
                log.info("{} [정보 없음] id : {}", context, id);
                return;
            }

            if (user.isSuspended()) {
                log.warn("{} [이미 삭제(추방) 회원] id : {}, status : {}",context, id, user.getStatus());
                return;
            }
            user.markSuspended(adminUser.getId());
        });
        log.info("{} [완료] dtoIds : {}, adminId : {}",context, dto.getIds(), adminUser.getId());
